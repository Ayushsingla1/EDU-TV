        ...
        <div className="w-full flex justify-center items-center relative">
          ...
          <div className="absolute bottom-[-20px] z-20 flex-col flex gap-y-1 left-[50%] translate-x-[-50%]">
            <h1 className="font-hanalei text-6xl flex justify-center items-center gap-x-3 text-white">
              WELCOME{" "}
              <span className="text-[#1ff000] text-3xl">
                {connectAccount.address?.slice(0, 7) +
                  "...." +
                  connectAccount.address?.slice(-7)}
              </span>
            </h1>
          ...
          <div className="absolute bottom-[-12px] z-10 left-[50%] rounded-full translate-x-[-50%] w-[280px] blur-[80px] h-[70px] bg-pink-500">
            hi this is dev
          </div>
        </div>
        <div className="px-10 w-10/12 max-w-[1600px] flex flex-col gap-y-2">
          <div className="flex justify-between text-xl">
            <h2 className="font-hanalei text-3xl text-[#1EFF00]">
              RENTED MOVIES
            </h2>
          ...
          <div className="flex w-full gap-5">
            {videos.slice(0, 5).map((video: video | any, index) => {
              return <MovieCard key={index} video={video} />;
            })}
          </div>
        </div>
      </div>
